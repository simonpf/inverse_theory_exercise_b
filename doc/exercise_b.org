...
**** Error Analysis
     ...
     \begin{align}
      MAPE &= \frac{100\%}{n} \sum_{i = 1}^n \frac{|\hat{x}(\mathbf{y}_i) - x_i|}{\hat{x}(\mathbf{y}_i)} \\ 
      MPE &= \frac{100\%}{n} \sum_{i = 1}^n \frac{\hat{x}(\mathbf{y}_i) - x_i}{\hat{x}(\mathbf{y}_i)}
     \end{align}
     
     as a function of $\hat{x}(\mathbf{y}_i) > 0$ for the simulated measurements $\mathbf{y}_i, x_i$
     ...
     deviation of the posterior as a function of $\hat{x}(\mathbf{y}_i)$.
